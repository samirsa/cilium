...
const pdpHdrLen = 9
const pdpMaxLen = 268435456 // 256 MB, per spec
...
func (rule *PDPRule) Matches(data interface{}) bool {
	...
	if rule.queryActionExact != "" && rule.queryActionExact != parts[2] {
		log.Debugf("PDPRule: query_action mismatch %v, %s", rule.queryActionExact, parts[1])
		return false
	}
	if len(parts[3]) > 0 &&
		rule.tableRegexCompiled != nil &&
		!rule.tableRegexCompiled.MatchString(parts[3]) {
		log.Debugf("PDPRule: table_regex mismatch '%v', '%s'", rule.tableRegexCompiled, parts[3])
		return false
	}
...
// PDPRuleParser parses protobuf L7 rules to enforcement objects
// May panic
func PDPRuleParser(rule *cilium.PortNetworkPolicyRule) []L7NetworkPolicyRule {
	...
	for _, l7Rule := range l7Rules.GetL7Rules() {
		...
		if len(cr.queryActionExact) > 0 {
			...
			if res == invalidAction {
				ParseError(fmt.Sprintf("Unable to parse L7 pdp rule with invalid query_action: '%s'", cr.queryActionExact), rule)
		...
		log.Debugf("Parsed PDPRule pair: %v", cr)
		rules = append(rules, &cr)
	}
	return rules
}

type PDPParserFactory struct{}

var pdpParserFactory *PDPParserFactory

func init() {
	log.Debug("init(): Registering pdpParserFactory")
	RegisterParserFactory("pdp", pdpParserFactory)
	RegisterL7RuleParser("pdp", PDPRuleParser)
}

type PDPParser struct {
...
func (pf *PDPParserFactory) Create(connection *Connection) Parser {
	log.Debugf("PDPParserFactory: Create: %v", connection)

	p := PDPParser{connection: connection}
...
func (p *PDPParser) OnData(reply, endStream bool, dataArray [][]byte) (OpType, int) {
	...
	if len(data) < pdpHdrLen {
		// Partial header received, ask for more
		needs := pdpHdrLen - len(data)
	...
	if reply {
		...
		pdpParseReply(p, data[0:(pdpHdrLen+requestLen)])

		log.Debugf("reply, passing %d bytes", (pdpHdrLen + requestLen))
		return PASS, (pdpHdrLen + int(requestLen))
	}

	err, paths := pdpParseRequest(p, data[0:(pdpHdrLen+requestLen)])
	if err != 0 {
	...
	if !matches {
		...
		return DROP, int(pdpHdrLen + requestLen)
	}

	return PASS, int(pdpHdrLen + requestLen)
}

// A full response (header + body) to be used as an
// "unauthorized" error to be sent to pdp client as part of policy
...
// "unprepared" error to be sent to pdp client if proxy
...
func parseQuery(p *PDPParser, query string) (string, string) {
	var action string
	var table string
	...
	// characters for pdp, we fail parsing.
...
func pdpParseRequest(p *PDPParser, data []byte) (OpError, []string) {
...
func pdpParseReply(p *PDPParser, data []byte) {
